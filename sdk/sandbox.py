...
class Sandbox:
    ...
    def _do_create(self):
        ...
        try:
            ...
            self.sandbox_id = result.get(SANDBOX_ID_KEY)
            if not self.sandbox_id:
                raise RuntimeError("Sandbox creation failed: missing sandbox_id in response")
    ...
    def create(cls, snapshot_id: Optional[str] = None, **kwargs) -> "Sandbox":
        if kwargs.get("sandbox_id") is not None:
            raise ValueError("sandbox_id is generated by conchd")
