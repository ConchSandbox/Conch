...
1. `virtiofsd` 启动成功后立即启动一个 goroutine，唯一地执行底层 `processHandle.Wait()`；当前实现由 child handle 调用 `cmd.Wait()`。
...
本方案刻意保持控制面范围小：不设计 virtiofsd 在线重启，不增加 bbolt terminal 状态，不扩展 API/CLI。当前 conchd 重启恢复直接调用 `CleanupStaleResources` 清理旧 Sandbox 和旧 virtiofsd，不接管旧进程继续运行；`processHandle` 抽象保留，但本次运行期实现只有 Conch 自己启动的 child process。
...
运行中的 virtiofsd 由当前 conchd 通过 `exec.Cmd.Start()` 启动，conchd 是父进程，可以调用 `cmd.Wait()` 回收并取得退出状态。运行期 monitor 只处理这种 child process。

conchd 重启不会 rehydrate 或接管旧 Sandbox。恢复入口直接调用 `CleanupStaleResources`：扫描并终止遗留 virtiofsd，再清理遗留 volume目录。`Device.PID/StartTime`、`isOurVirtiofsd` 和 `os.FindProcess` 只属于无active owner时的清理fallback，不会构造 `ProcessWatch`，也不会进入 Sandbox运行期状态机。
...
type processHandle interface {
    ...
    // Close 释放底层句柄，不负责 Sandbox 生命周期判断。
    Close() error
}
```

`Wait() error` 不够，因为 child 的 `cmd.Wait()` 返回 `*exec.ExitError` 时，进程已经退出并已被回收，应该表达为 `Exited=true, Cause=exitErr`；其他不能证明进程已经回收的 Wait错误则需要表达为 `Exited=false, Cause=err`。
...
`virtiofsProcess` 不直接操作 `*exec.Cmd`，也不对具体handle做 type switch。底层进程细节留在 `processHandle` 实现中；当前唯一实现是 `childProcess`。
...
正确契约是：monitor 先保存不可变 Result，再关闭 `observeDone`。channel close 向全部观察者广播；调用者在 `<-Done()` 后读取 Result。Result 中 `Exited=false` 也必须通知外层，因为无法证明依赖进程状态本身就是运行期故障。

`PreparedSandbox.Watch` 还承担 active process identity：即使 monitor 已从 backend map 删除记录，Sandbox cleanup 仍通过这个 Watch 找到同一个 `virtiofsProcess`。active cleanup 不能退回只凭 sandboxID重新查 map。
...
没有 volume mount 时返回零值 `PreparedSandbox`，`Watch == nil`。运行期路径从刚启动的 `exec.Cmd` 创建 child handle，再包装成 `virtiofsProcess + ProcessWatch`。

active cleanup 接收完整 `PreparedSandbox` 并调用其中精确 process 的 Close，再清理 bind mount/runtime dir。conchd启动残局直接走独立的 `CleanupStaleResources`；它不创建运行期owner或watcher。
...
Prepare 成功后设置 `volumeOwnedByCreate=true`；注册进 `sbx.cleanup` 的 closure 必须捕获完整 `prepared`，只有注册成功后才改为 false。这样无论异常发生在 VMM 创建前、VMM 创建中还是 cleanup 注册前，都不会丢失或重复清理 volume，也不会在 monitor 删除backend map记录后丢失active process owner。
...
| `internal/volume/manager.go` | Prepare 返回 `PreparedSandbox`；active Cleanup消费完整 owner |
...
3. 在 child adapter之上实现统一 `virtiofsProcess`、Done/Result和 strict-once Close。
4. 让 `waitUnixSocket` 同时观察 process Done，补齐 Prepare 阶段测试。
5. 将 volume Prepare 返回值改为 `PreparedSandbox`。
6. 将 `sandboxEntry` 改为 `CREATING/READY/SUSPENDED/STOPPING/EXITED`。
7. 缩短 Create、Delete、VMM exit 对 `entry.mu` 的持锁范围。
8. 启动 channel watcher并实现创建期 dependencyErr/cancel。
9. 统一 volume exit、VMM exit、Delete 的 cleanup ownership。
10. 补 child、wrapper、Sandbox race test和真实 VOL-011。

## 11. 单元测试矩阵

### 11.1 Process和child adapter

- child 的 Wait返回 nil或 ExitError时都设置 `Exited=true`。
- child Kill/ConfirmExit不产生第二次 `cmd.Wait()`。
- `processHandle` 抽象由 child adapter实现，上层不接触 `exec.Cmd`。
...
- 本次不实现旧进程adoption或Sandbox rehydrate；conchd重启直接调用 `CleanupStaleResources` 清理遗留资源。
...
- [ ] childProcess实现最小 processHandle接口。
- [ ] virtiofsProcess和 Sandbox Manager不直接接触 `exec.Cmd`。
- [ ] child只有一次 cmd.Wait。
- [ ] virtiofsd 从 Start成功起始终有唯一 Wait/observe goroutine。
