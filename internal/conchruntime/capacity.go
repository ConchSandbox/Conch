...
// CapacityLimits bounds running and starting sandboxes together. Zero limits
// select no sandbox-count cap and the host's CPU or memory capacity.
// Reservations are held until runtime teardown completes.
...
func (c *Capacity) reserve(id string, cpus, memoryMB int64) error {
	...
	if cpus <= 0 || memoryMB <= 0 || (c.limits.MaxSandboxes > 0 && int64(len(c.reservations)) >= c.limits.MaxSandboxes) ||
