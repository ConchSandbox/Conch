...
func (s *Server) list(w http.ResponseWriter, r *http.Request) {
	...
	metadataValues, err := url.ParseQuery(query.Get("metadata"))
	if err != nil {
		writeError(w, 400, "invalid metadata filter")
		return
	}
	// E2B parses metadata into a map[string]string. If a key is repeated,
	// the last value replaces the earlier one rather than requiring all values
	// to match.
	metadata := make(map[string]string, len(metadataValues))
	for key, values := range metadataValues {
		if len(values) > 0 {
