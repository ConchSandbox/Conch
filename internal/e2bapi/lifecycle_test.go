...
// advances the record head and invokes the callback under the content lease;
// publication and record persistence run against real components.
...
func TestExpiryAutoPausesTimeoutActionPause(t *testing.T) {
	...
	rec := before
	rec.TimeoutAction = runtimeapi.TimeoutActionPause
