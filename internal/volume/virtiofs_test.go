...
func TestCleanupWaitsForVirtiofsProcessExit(t *testing.T) {
	...
	backend.procs.Store(sandboxID, &virtiofsProcess{process: cmd.Process, done: done})
	if backend.Exited(sandboxID) != done {
		t.Fatal("Exited did not return the process exit notification")
	}
	cleanupResult := make(chan error, 1)
	go func() {
		cleanupResult <- backend.Cleanup(sandboxID, nil)
	}()
