...
func runManagedCowTestProcess(socketPath string) int {
	...
	defer stop()
	server := newServer(socketPath, Capabilities{IncrementalMemory: CapabilityUnsupported})
...
func TestStartProcessWaitsForCapabilitiesAndClosesChild(t *testing.T) {
	...
	if capabilities.IncrementalMemory != CapabilityUnsupported {
