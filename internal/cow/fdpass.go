...
func readFrame(conn *net.UnixConn, message any) ([]int, error) {
	return readFrameWithOOBSize(conn, message, defaultFrameOOBLen)
}

func readFrameWithOOBSize(conn *net.UnixConn, message any, oobSize int) ([]int, error) {
	header := make([]byte, 4)
	...
	if err := decoder.Decode(&struct{}{}); err != io.EOF {
		closeFDs(fds)
		if err == nil {
			return nil, fmt.Errorf("decode frame: multiple JSON values")
		}
		return nil, fmt.Errorf("decode frame trailing data: %w", err)
	}
	return fds, nil
}
...
func validateResponse(requestID string, response Response, fds []int, expectedFDs int) error {
