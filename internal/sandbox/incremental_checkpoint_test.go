...
func TestIncrementalCaptureCreatesBaseEpochAndResumes(t *testing.T) {
	backend := &recordingIncrementalBackend{}
...
func TestIncrementalCaptureCreatesDeltaFromQueryAndResetBitmap(t *testing.T) {
	...
	if !reflect.DeepEqual(backend.operations, []string{"state", "mappings", "dirty", "delta", "manifest"}) {
		t.Fatalf("operations = %v", backend.operations)
	}
}
...
func (backend *recordingIncrementalBackend) QueryDirtyBitmap(IncrementalMemoryRuntime) (driver.MemoryDirtyBitmap, error) {
	return driver.MemoryDirtyBitmap{}, backend.record("dirty")
}
