...
type Manager struct {
	...
	// UnexpectedExitHandler is invoked once per runtime after an unexpected
	// sandbox exit has been cleaned up and persisted. Configure it before
	// creating sandboxes; it runs without the lifecycle lock held.
	...
	// Configure it before creating sandboxes. It runs under the sandbox's
	// lifecycle lock and must not acquire lifecycle locks or call back into Manager.
	BeforeRuntimeRelease func(sandboxID string)
}

const createCleanupTimeout = 10 * time.Second

// UnexpectedExitHandler observes asynchronous runtime exits. runtimeID
// identifies the exited allocation when the same public sandbox ID has
// already been reused by a newer create.
