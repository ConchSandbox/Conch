...
func New(cfg *config.Config) (*Daemon, error) {
	...
	if err := s.runtimeService.HydrateSandboxStates(ctx); err != nil {
		cancel()
		_ = host.Close()
		_ = store.Close()
		return nil, fmt.Errorf("hydrate sandbox guard states: %w", err)
	}
...
func (s *Daemon) handleCreateSandbox(w http.ResponseWriter, r *http.Request) {
	...
	if req.SandboxId != "" {
		http.Error(w, "sandbox_id is generated by conchd", http.StatusBadRequest)
		return
	}
...
func (s *Daemon) handleDeleteSandbox(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		...
		http.Error(w, "Failed to delete sandbox: "+err.Error(), status)
		return
	}
...
func (s *Daemon) handlePauseSandbox(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		...
		http.Error(w, "Failed to pause sandbox: "+err.Error(), status)
		return
	}
...
func (s *Daemon) convertImage(ctx context.Context, req convertImageRequest, kernelPath, initrdPath string) (convertImageResponse, error) {
	...
	if req.Snapshot {
		...
		createResult, err := s.runtimeService.CreateSandbox(ctx, runtimeapi.SandboxCreateOptions{
			Namespace: namespace,
			ImageName: bootResp.ImageName,
