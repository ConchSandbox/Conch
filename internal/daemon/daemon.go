...
// CheckpointSandbox implements cri.Checkpointer: it exports a running sandbox to
// an OCI snapshot image and pushes it to a registry for cross-node restore.
func (s *Daemon) CheckpointSandbox(ctx context.Context, opts cri.CheckpointOptions) (cri.CheckpointResult, error) {
	...
	// Pause + commit into a local OCI image; this terminates the source sandbox.
	...
	if path := strings.TrimSpace(opts.ArchivePath); path != "" {
		if err := s.exportCheckpointArchive(ctx, opts.Namespace, opts.ImageRef, path); err != nil {
			return cri.CheckpointResult{}, fmt.Errorf("write checkpoint archive %s: %w", path, err)
		}
	}
...
// exportCheckpointArchive writes the snapshot image to path as an OCI archive via a
// temp file + rename, so a caller never observes a partial archive.
func (s *Daemon) exportCheckpointArchive(ctx context.Context, namespace, imageRef, path string) error {
	if err := os.MkdirAll(filepath.Dir(path), 0o755); err != nil {
		return fmt.Errorf("create archive dir: %w", err)
	}
	tmp, err := os.CreateTemp(filepath.Dir(path), ".checkpoint-*.tar.tmp")
	if err != nil {
	...
	if err := s.runtimeService.ExportImageArchive(ctx, tmp, imageSvc.ExportArchiveRequest{
		ImageName: imageRef,
		Namespace: s.resolveNamespace(namespace),
	}); err != nil {
		tmp.Close()
		return fmt.Errorf("export snapshot archive: %w", err)
	}
