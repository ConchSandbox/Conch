...
type BootLayout struct {
	...
	MemorySnapshotRoot string // mount root exposing the memory component
...
func (w *BootLayout) VMMStatePath() string {
	if w == nil || w.MemoryLayout == MemoryLayoutNone || strings.TrimSpace(w.MemorySnapshotRoot) == "" {
		return ""
	}
	return filepath.Join(w.MemorySnapshotRoot, common.VMMStateDir)
}
...
func (s *Server) CreateBootLayout(
	...
	// Step 4: prepare the VMM state directory.
	if err = prepareVMMStateDirectory(layout); err != nil {
		return nil, fmt.Errorf("prepare VMM state directory failed: %v", err)
	}
