...
func (s *StratovirtClient) QueryMemoryDirtyBitmap() (driver.MemoryDirtyBitmap, error) {
	response, err := s.executeQMPCommandWithResponse("query-mem-dirty-bitmap", map[string]any{"clear": false})
	if err != nil {
...
func (s *StratovirtClient) ClearMemoryDirtyBitmap(generation uint64) error {
	return s.executeQMPCommand("clear-mem-dirty-bitmap", map[string]any{"generation": generation})
}
