...
func (s *StratovirtClient) BuildStartCmd(args *driver.ResourceArgs, restore bool) (string, error) {
	...
	if args.IncrementalRestore != nil {
		memoryFD = args.IncrementalRestore.MemoryFD
		uffdSocketPath = args.IncrementalRestore.UFFDSocketPath
	}
	...
	if restore {
		...
		if args.MemoryFormat == "incremental-v1" {
			if args.IncrementalRestore == nil {
				return "", fmt.Errorf("incremental restore requires incremental restore arguments")
			}
			if args.IncrementalRestore.MemoryFD < 3 {
				return "", fmt.Errorf("incremental restore requires an inherited memory fd")
			}
			if strings.TrimSpace(args.IncrementalRestore.UFFDSocketPath) == "" {
...
func (s *StratovirtClient) PauseVM() error {
	...
	for i := 0; i < 20; i++ {
		...
		lastStatus, lastErr = s.queryStatus()
		logger.Debug("VM status after stop", ulog.F("status", lastStatus), ulog.F("error", lastErr))
		if lastErr == nil && (lastStatus == "paused" || lastStatus == "stopped") {
	...
	if lastErr != nil {
		return fmt.Errorf("timeout waiting for vm to pause: last status query failed: %w", lastErr)
	}
	return fmt.Errorf("timeout waiting for vm to pause: last status %q", lastStatus)
}
...
func (s *StratovirtClient) CreateExternalMemorySnapshot(snapfilePath string) error {
	status, err := s.queryStatus()
	if err != nil {
		return fmt.Errorf("failed to query vm status before external memory snapshot: %w", err)
	}
	if status != "paused" && status != "stopped" {
		return fmt.Errorf("external memory snapshot requires a paused vm, got status %q", status)
	}
