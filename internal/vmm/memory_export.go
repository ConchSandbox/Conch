...
func ExportMemoryByPageState(
	...
	return exportMemory(ctx, pid, memorySize, blockSize, state.PageSize, mappings, selector, sink)
}
...
func ExportMemoryByDirtyBitmap(
	...
	return exportMemory(ctx, pid, memorySize, blockSize, dirty.PageSize, mappings, selector, sink)
}
...
func exportMemory(
	...
	buffer := make([]byte, int(memoryExportBatchSize))
	...
	for offset := uint64(0); offset < memorySize; {
		...
		case pageRead:
			...
			length := blockSize
			for length < memoryExportBatchSize && offset+length < mappingEnd && selector(offset+length) == pageRead {
				length += blockSize
			}
