...
type Process struct {
	...
	// inheritedFiles owns the parent-side descriptors passed to the VMM until the child process starts.
	inheritedFiles []*os.File
	// resumeStartedHook waits for the external memory service after the restore process starts.
...
func NewProcess(
	...
	incrementalRestore := vmmResourceArgs.IncrementalRestore
	var inheritedFiles []*os.File
	if incrementalRestore != nil && incrementalRestore.MemoryFile != nil {
		inheritedFiles = append(inheritedFiles, incrementalRestore.MemoryFile)
	}
	...
	for _, file := range inheritedFiles {
		incrementalRestore.MemoryFD = appendInheritedFile(cmd, file)
	}
	var resumeStartedHook func(context.Context) error
	if incrementalRestore != nil {
		resumeStartedHook = incrementalRestore.ResumeStartedHook
	}
	p := Process{
		SandboxId:         sandboxId,
		...
		resumeStartedHook: resumeStartedHook,
	}
...
func (p *Process) Restore(ctx context.Context, snapshotPath string) error {
	...
	if p.resumeStartedHook != nil {
		if err := p.resumeStartedHook(ctx); err != nil {
			vmmStopErr := p.Stop()
