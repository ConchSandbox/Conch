	...
	for _, tt := range []struct {
		...
		t.Run(tt.vmm, func(t *testing.T) {
			...
			if tt.vmm == "stratovirt" {
				annotations[AnnotationMemoryFormat] = MemoryFormatFull
			}
...
func TestInspectBootIndexStratoVirtMemoryFormat(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "full", vmm: "stratovirt", format: MemoryFormatFull, wantFormat: MemoryFormatFull},
		...
		{name: "cloud hypervisor annotated", vmm: "cloud-hypervisor", format: MemoryFormatFull, wantError: true},
	}
...
func TestBuildIncrementalMemoryComponentAppendsOneLayer(t *testing.T) {
	...
	first, err := BuildIncrementalMemoryComponentInContent(ctx, store, ocispec.Descriptor{}, writeComponentRoot(t, dir, "epoch-a"))
	if err != nil {
		t.Fatal(err)
	}
	second, err := BuildIncrementalMemoryComponentInContent(ctx, store, first, writeComponentRoot(t, dir, "epoch-b"))
	if err != nil {
		t.Fatal(err)
	}
