...
func ApplySandboxNetworkPolicy(ctx context.Context, slot *Slot, cfg *SandboxNetworkConfig) error {
	if err := ValidateSandboxNetworkInputConfig(ctx, cfg); err != nil {
		return err
	}
	if !isNetworkConfigNonEmpty(cfg) {
		return nil
	}
	return replaceSandboxNetworkPolicy(ctx, slot, cfg)
}
...
func replaceSandboxNetworkPolicy(ctx context.Context, slot *Slot, cfg *SandboxNetworkConfig) error {
	...
	return runInNetNSPath(ctx, slot.NetNSPath(), func() error {
		...
		return restoreSandboxNetworkPolicy(ctx, slot.TapName(), !egressHook, !ingressHook, cfg)
	})
}

func restoreSandboxNetworkPolicy(ctx context.Context, tapName string, addEgressHook, addIngressHook bool, cfg *SandboxNetworkConfig) error {
...
func sandboxPolicyRules(cfg *SandboxNetworkConfig, ingress bool) [][]string {
	...
	for _, entry := range policyDestinations(deny) {
		rules = append(rules, []string{addressFlag, entry, "-j", "REJECT"})
	}
	for _, entry := range policyDestinations(allow) {
...
func policyDestinations(entries []string) []string {
	...
	for _, entry := range entries {
		if normalized, ok := normalizePolicyDestination(entry); ok {
