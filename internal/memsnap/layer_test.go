...
func TestCreateBaseLayerRejectsLayerDirectorySymlink(t *testing.T) {
	root := t.TempDir()
	outside := t.TempDir()
	if err := os.Symlink(outside, filepath.Join(root, LayerDirName)); err != nil {
		t.Fatal(err)
	}
	_, err := CreateBaseLayer(root, DefaultBlockSize, DefaultBlockSize, func(sink PageSink) error {
		return sink.WriteZeroPage(0)
	})
	if err == nil {
		t.Fatal("CreateBaseLayer followed a layer-directory symlink")
	}
	if _, err := os.Stat(filepath.Join(outside, "0.mem")); !os.IsNotExist(err) {
		t.Fatalf("outside layer exists: %v", err)
	}
}
