.PHONY: build clean test fmt vet lint help gen-proto gen-proto-go gen-proto-py mod-tidy mod-vendor install build-conch-init-initramfs
...
gen-proto-go: ## Generate Go protobuf code
	...
	@protoc \
		...
		--go-grpc_out=$(CONCH_GO_PROTO_DIR) \
...
build-conch-init-initramfs: gen-proto-go ## Build Alpine initramfs that runs conch-init as PID 1
